...
class MILdataset(data.Dataset):
    def __init__(self, cfgs, split='train'):
        ...
        if split == 'train':
            self.bag_info = pd.read_csv(osp.join(self.cfgs["train_data"], 'train_final.CSV'))
            self.bag_list = osp.join(self.cfgs["train_data"], "pt_files")
        else:
            self.bag_info = pd.read_csv(osp.join(self.cfgs["test_data"], 'test_final.CSV'))   # all bag
    ...
    def top_k_select(self, pred, is_in_bag=False):#mixabmil
        ...
        for index in range(len(self.bag_info)):
            ...
            return_result[index].append(_index[-int(k/2):] + _index[: int(k/2)])# + _score[-k:])# + _index[: k])
            if is_in_bag:
                # k = self.k if self.k < _index.shape[0] else _index.shape[0]
                self.selected_bag_info[index_selected_bag[_count]] = {"slide_id": slide_id, "label": target, "instance_index": _index[-int(k/2):] + _index[: int(k/2)]}# }
                _count += 1
            else:
                for ii in range(k):
